...
//保存self的父视图，用于复位
@property(readwrite,nonatomic,strong)UIView *restSuperView;

@property (readwrite, nonatomic, copy, setter = setCompletion:) void (^completion)(BOOL finished);
//保留初始值
@property (readwrite, nonatomic, assign, setter = setOriginalPoint:) CGPoint originalPoint;
@end
...
- (void)dragPanAction:(UIPanGestureRecognizer *)pan {
    ...
    CGPoint panPoint = CGPointMake(topFloorPoint.x + translation.x,topFloorPoint.y + translation.y);
...
- (void) cellAnimations:(NSArray*)cells withGesture:(UIGestureRecognizer*)gesture {
    ...
    for (NSInteger j=0; j<cells.count; j++) {
        ...
        pathAnimation.path = [self makeRandomPath: shape AngleTransformation:j].CGPath;//设置碎片路径
